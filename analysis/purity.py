...
def purity_he3_TPC(infile_path: str, output_file: TFile):
    ...
    x = RooRealVar('x', 'x', ymin, ymax)
    roofitter = Roofitter(x, ['exp_offset', 'exp_mod_gaus', 'gaus']) # fit with a gaussian and an exponential bkg
    print('params: \n', roofitter.pdf_params)
    roofitter.init_param('exp_mod_gaus_1_mean', 0.2, 0., 1.5)
    roofitter.init_param('exp_mod_gaus_1_sigma', 0.2, 0.05, 2)
    roofitter.init_param('exp_0_alpha', 0.5, 0., 10.)
    roofitter.init_param('exp_0_offset', -0.5, -10., 0.)
    ...
    for pt_bin in range(h2_nsigmaTPC.GetXaxis().FindBin(xmin), h2_nsigmaTPC.GetXaxis().FindBin(xmax)):
        ...
        h_nsigmaTPC.Rebin(4)
        pt = h2_nsigmaTPC.GetXaxis().GetBinCenter(pt_bin)
        funcs_to_fit = ['gaus_2', 'exp_mod_gaus_1', 'exp_0'] if pt < 2.5 else ['exp_0', 'exp_mod_gaus_1']
        ...
        pts.append(np.abs(pt))
        signal.append(integrals['exp_mod_gaus_1'])
        ibkg = integrals['gaus_2'] + integrals['exp_0'] if pt < 2.5 else integrals['exp_0']
        bkg.append(ibkg)
        itot = integrals['exp_mod_gaus_1'] + ibkg
        tot.append(itot)
        purity.append(integrals['exp_mod_gaus_1'] / itot)
