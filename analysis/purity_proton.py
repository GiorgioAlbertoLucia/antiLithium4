...
def purity_proton_TPC(output_file: TFile):
    ...
    h2_nsigmaTPC_info = HistLoadInfo('/Users/glucia/Projects/ALICE/data/lithium/same/AnalysisResults_LHC24ar_pass1_same.root', 'he3hadronfemto/QA/h2NsigmaHadronTPC_preselection')
...
def purity_proton_TOF(output_file: TFile):
    ...
    h2_nsigmaTOF_info = HistLoadInfo('output/LHC24PbPb/data_visual_selectionsPr.root', 'TOF/NSigmaTOFvsPtHad')
...
def purity_he3_TPC(output_file: TFile):
    ...
    h2_nsigmaTPC_info = HistLoadInfo('/Users/glucia/Projects/ALICE/data/lithium/same/AnalysisResults_LHC24ar_pass1_same.root', 'he3hadronfemto/QA/h2NsigmaHe3TPC_preselection')
    h2_nsigmaTPC = histogram.load_hist(h2_nsigmaTPC_info)
    xmin = 1.6
    xmax = 3.5
    ymin = -5.
    ...
    roofitter.init_param('gaus_1_sigma', 0.2, 0.05, 2)
    roofitter.init_param('exp_0_alpha', 0.5, 0., 10.)
    roofitter.init_param('exp_0_offset', -0.5, -10., 0.)
    ...
    for pt_bin in range(h2_nsigmaTPC.GetXaxis().FindBin(xmin), h2_nsigmaTPC.GetXaxis().FindBin(xmax)):
        ...
        h_nsigmaTPC.Rebin(4)
        pt = h2_nsigmaTPC.GetXaxis().GetBinCenter(pt_bin)
        funcs_to_fit = ['gaus_2', 'gaus_1'] if pt < 2.5 else ['exp_0', 'gaus_1']
        ...
        pts.append(np.abs(pt))
        signal.append(integrals['gaus_1'])
        ibkg = integrals['gaus_2'] if pt < 2.5 else integrals['exp_0']
        bkg.append(ibkg)
        itot = integrals['gaus_1'] + ibkg
        tot.append(itot)
        purity.append(integrals['gaus_1'] / itot)
